...
# Global HMAC-SHA256
# Random container ID
# Optional original-file deletion after verification
#
# ============================================================
...
def derive_subkeys(
    ...
    Container ID is used as HKDF salt/context.
    """
...
def read_header(file_obj):
    ...
    container_id = file_obj.read(CONTAINER_ID_SIZE)
...
def read_record(file_obj):
    ...
    payload = file_obj.read(payload_size)
...
def calculate_total_size(entries):
...
def parse_metadata(plaintext):
    ...
    # Windows drive-relative / malformed path protection.
...
def validate_encryption_inputs(
    inputs,
    output_path,
    delete_originals,
):
    ...
    # Prevent duplicate / nested inputs.
        ...
        for j, second in enumerate(normalized):
            ...
            try:
                ...
                if first.is_dir():
                    ...
                    raise ValueError(
                        "Selected inputs overlap.\n\n"
                        f"'{second}' is inside '{first}'.\n\n"
                        "Please select only the parent folder "
                        "or the individual items, not both."
                    )
    ...
    if output in normalized:
        ...
        raise ValueError(
            "The output container cannot be the same "
            "as an input file."
        )
        ...
        for input_path in normalized:
            ...
            if input_path.is_dir():
                ...
                try:
                    ...
                    raise ValueError(
                        "The output container cannot be placed "
                        "inside a source folder when automatic "
                        "deletion is enabled."
                    )
                ...
                if output == input_path:
                    ...
                    raise ValueError(
                        "The output container cannot overwrite "
...
def encrypt_container(
    ...
    entries = enumerate_inputs(inputs)
    ...
    if status_callback:
        ...
        status_callback(
            ...
            "Container is ready for verification."
        )
...
def verify_global_hmac(
    ...
    with open(input_path, "rb") as source:
        ...
        remaining = bytes_to_authenticate
...
def validate_container(
    ...
    with open(input_path, "rb") as source:
        ...
        salt, container_id = read_header(source)
...
def delete_original_inputs(
    inputs,
    status_callback=None,
):

    """
    Removes original files/folders only after
    successful container creation and full verification.

    IMPORTANT:
    This removes filesystem entries. It does NOT guarantee
    physical destruction of old SSD/HDD sectors.
    """
        ...
        if path.is_file():
            ...
            try:
                ...
                raise RuntimeError(
                    ...
                    "and verified, but the original file "
            ...
            try:
                ...
                raise RuntimeError(
                    "Container was successfully created "
                    "and verified, but the original folder "
                    f"could not be deleted:\n\n"
                    f"{path}\n\n"
                    f"Reason: {exc}"
                )
    ...
    def run(self):
        ...
        try:
            ...
            if self.operation == "encrypt":
                ...
                self.status.emit(
                    "Encryption finished. "
                    "Verifying container before "
                    "any original data is deleted..."
                )
...
class PoliCryptWindow(QMainWindow):
    ...
    def build_ui(self):
        ...
        self.delete_originals_checkbox = QCheckBox(
            "Delete original files/folders after successful encryption"
        )

        self.delete_originals_checkbox.setChecked(
            False
        )

        delete_layout.addWidget(
            self.delete_originals_checkbox
        )

        delete_info = QLabel(
            "Original data is deleted only after the generated "
            "container passes full authentication and integrity "
            "verification."
        )

        delete_info.setWordWrap(
            True
        )
        ...
        if self.encrypt_radio.isChecked():
            ...
            if delete_originals:
                ...
                answer = QMessageBox.warning(
                    ...
                    "PoliCrypt will first create and fully "
                    "authenticate the encrypted container.\n\n"
                    "ONLY if verification succeeds will the "
                    "original files/folders be deleted.\n\n"
                    "The original data will no longer be "
    ...
    def operation_success(
        ...
        if self.encrypt_radio.isChecked():
            ...
            if self.delete_originals_checkbox.isChecked():

                QMessageBox.information(
                    self,
                    "PoliCrypt",
                    "Encryption completed successfully.\n\n"
                    "The container passed full authentication "
                    "and integrity verification.\n\n"
                    "The original data was deleted.\n\n"
                    f"{result}",
                )

                self.status_label.setText(
                    "Encryption and verified deletion completed."
                )
    ...
    def operation_failure(
        ...
        self.status_label.setText(
            "Operation failed. Original data was not "
    ...
    def set_busy(
        ...
        self.delete_originals_checkbox.setEnabled(
            not busy
        )
